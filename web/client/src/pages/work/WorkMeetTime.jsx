...
function WorkMeetTime() {
  ...
  return (
    <div className="page-container">
      ...
      <h2 style={{ fontSize: 18, marginBottom: 16 }}>時段管理</h2>
      ...
      {days.map(d => (
        <div key={d.DAY_ID} className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h4>{d.day}</h4>
            <button className="btn-link" style={{ color: '#ff4d4f' }} onClick={() => handleDelete(d.DAY_ID)}>刪除</button>
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
            {(d.times || []).map((t, j) => (
              <span key={j} style={{ background: 'var(--bg-elevated)', padding: '4px 8px', borderRadius: 4, fontSize: 13, display: 'inline-flex', gap: 8, alignItems: 'center' }}>
                {t.start}-{t.end} (限{t.limit}人)
                <button className="btn-link" style={{ fontSize: 12 }} onClick={() => setEditingTime({
                  dayId: d.DAY_ID, mark: t.mark, day: d.day, start: t.start, end: t.end, enrolled: t.stat?.succCnt || 0,
                })}>更改</button>
              </span>
            ))}
          </div>
