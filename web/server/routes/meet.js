...
router.get('/list', (req, res) => {
  ...
  const rows = db.prepare('SELECT * FROM meets WHERE MEET_STATUS IN (1, 9) ORDER BY MEET_ORDER ASC, MEET_ADD_TIME DESC LIMIT ?').all(limit)
...
router.get('/days', (req, res) => {
  ...
  const rows = db.prepare('SELECT * FROM days WHERE day >= ? AND day <= ? ORDER BY day ASC').all(start || '', end || '9999-12-31')
...
router.get('/by-day', (req, res) => {
  const { day } = req.query
  const dayRows = db.prepare('SELECT * FROM days WHERE day = ?').all(day)
  const result = []
  for (const d of dayRows) {
    const meet = db.prepare('SELECT * FROM meets WHERE MEET_ID = ? AND MEET_STATUS = 1').get(d.DAY_MEET_ID)
    if (meet) {
...
router.get('/my-joins', authUser, (req, res) => {
  const rows = db.prepare('SELECT * FROM joins WHERE JOIN_USER_ID = ? ORDER BY JOIN_ADD_TIME DESC').all(req.userId)
  rows.forEach(r => { r.JOIN_FORMS = parseJSON(r.JOIN_FORMS) })
  res.json({ data: rows })
})

// My join detail
router.get('/my-joins/:id', authUser, (req, res) => {
  const join = db.prepare('SELECT * FROM joins WHERE JOIN_ID = ? AND JOIN_USER_ID = ?').get(req.params.id, req.userId)
...
// Lesson logs
router.get('/lesson-logs', authUser, (req, res) => {
  const rows = db.prepare('SELECT * FROM lesson_logs WHERE LESSON_LOG_USER_ID = ? ORDER BY LESSON_LOG_ADD_TIME DESC').all(req.userId)
  res.json({ data: rows })
})
...
router.get('/:id/days', (req, res) => {
  ...
  const rows = db.prepare('SELECT * FROM days WHERE DAY_MEET_ID = ? AND day >= ? ORDER BY day ASC').all(req.params.id, today)
  rows.forEach(r => { r.times = parseJSON(r.times) })
  res.json({ data: rows })
})

// Join/Book appointment
router.post('/join', authUser, (req, res) => {
  const { meetId, day, timeMark, forms } = req.body
  const userId = req.userId

  const meet = db.prepare('SELECT * FROM meets WHERE MEET_ID = ?').get(meetId)
  ...
  const dayRow = db.prepare('SELECT * FROM days WHERE DAY_MEET_ID = ? AND day = ?').get(meetId, day)
  ...
  // Check duplicate
  const existing = db.prepare('SELECT JOIN_ID FROM joins WHERE JOIN_USER_ID = ? AND JOIN_MEET_ID = ? AND JOIN_MEET_DAY = ? AND JOIN_MEET_TIME_MARK = ? AND JOIN_STATUS = 1').get(userId, meetId, day, timeMark)
  ...
  db.prepare(`INSERT INTO joins (JOIN_ID, JOIN_USER_ID, JOIN_MEET_ID, JOIN_MEET_CATE_ID, JOIN_MEET_CATE_NAME, JOIN_MEET_TITLE, JOIN_MEET_DAY, JOIN_MEET_TIME_START, JOIN_MEET_TIME_END, JOIN_MEET_TIME_MARK, JOIN_CODE, JOIN_STATUS, JOIN_FORMS, JOIN_ADD_TIME, JOIN_EDIT_TIME)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?)`)
  ...
  // Deduct lesson
  db.prepare('UPDATE users SET USER_LESSON_TOTAL_CNT = USER_LESSON_TOTAL_CNT - 1, USER_LESSON_USED_CNT = USER_LESSON_USED_CNT + 1 WHERE USER_ID = ?').run(userId)

  // Log lesson change
  const logId = uuidv4()
  db.prepare(`INSERT INTO lesson_logs (LESSON_LOG_ID, LESSON_LOG_USER_ID, LESSON_LOG_MEET_ID, LESSON_LOG_TYPE, LESSON_LOG_CHANGE_CNT, LESSON_LOG_LAST_CNT, LESSON_LOG_NOW_CNT, LESSON_LOG_ADD_TIME, LESSON_LOG_EDIT_TIME)
    VALUES (?, ?, ?, 1, -1, ?, ?, ?, ?)`)
...
// Cancel my join
router.post('/my-joins/:id/cancel', authUser, (req, res) => {
  const join = db.prepare('SELECT * FROM joins WHERE JOIN_ID = ? AND JOIN_USER_ID = ? AND JOIN_STATUS = 1').get(req.params.id, req.userId)
  if (!join) return res.status(400).json({ msg: '預約不存在或已取消' })

  const meet = db.prepare('SELECT * FROM meets WHERE MEET_ID = ?').get(join.JOIN_MEET_ID)
  ...
  const now = Date.now()
  db.prepare('UPDATE joins SET JOIN_STATUS = 10, JOIN_EDIT_TIME = ? WHERE JOIN_ID = ?').run(now, join.JOIN_ID)
  ...
  // Restore lesson
  db.prepare('UPDATE users SET USER_LESSON_TOTAL_CNT = USER_LESSON_TOTAL_CNT + 1, USER_LESSON_USED_CNT = USER_LESSON_USED_CNT - 1 WHERE USER_ID = ?').run(req.userId)

  const user = db.prepare('SELECT USER_LESSON_TOTAL_CNT FROM users WHERE USER_ID = ?').get(req.userId)
  const logId = uuidv4()
  db.prepare(`INSERT INTO lesson_logs (LESSON_LOG_ID, LESSON_LOG_USER_ID, LESSON_LOG_MEET_ID, LESSON_LOG_TYPE, LESSON_LOG_CHANGE_CNT, LESSON_LOG_LAST_CNT, LESSON_LOG_NOW_CNT, LESSON_LOG_ADD_TIME, LESSON_LOG_EDIT_TIME)
    VALUES (?, ?, ?, 2, 1, ?, ?, ?, ?)`)
...
router.post('/my-joins/:id/reschedule', authUser, (req, res) => {
  ...
  const newDayRow = db.prepare('SELECT * FROM days WHERE DAY_MEET_ID = ? AND day = ?').get(join.JOIN_MEET_ID, day)
  ...
  const dup = db.prepare('SELECT JOIN_ID FROM joins WHERE JOIN_USER_ID = ? AND JOIN_MEET_ID = ? AND JOIN_MEET_DAY = ? AND JOIN_MEET_TIME_MARK = ? AND JOIN_STATUS = 1 AND JOIN_ID != ?')
    .get(req.userId, join.JOIN_MEET_ID, day, timeMark, join.JOIN_ID)
  if (dup) return res.status(400).json({ msg: '您已預約該時段' })

  const now = Date.now()
  const oldDayRow = db.prepare('SELECT * FROM days WHERE DAY_MEET_ID = ? AND day = ?').get(join.JOIN_MEET_ID, join.JOIN_MEET_DAY)
  if (oldDayRow && oldDayRow.DAY_ID === newDayRow.DAY_ID) {
    ...
    db.prepare('UPDATE days SET times = ? WHERE DAY_ID = ?').run(JSON.stringify(times), newDayRow.DAY_ID)
  } else {
    if (oldDayRow) {
      ...
      if (oldSlot && oldSlot.stat) {
        ...
        db.prepare('UPDATE days SET times = ? WHERE DAY_ID = ?').run(JSON.stringify(oldTimes), oldDayRow.DAY_ID)
      }
    }
    if (!newSlot.stat) newSlot.stat = { succCnt: 0, cancelCnt: 0, adminCancelCnt: 0 }
    newSlot.stat.succCnt = (newSlot.stat.succCnt || 0) + 1
    db.prepare('UPDATE days SET times = ? WHERE DAY_ID = ?').run(JSON.stringify(newTimes), newDayRow.DAY_ID)
  }

  db.prepare('UPDATE joins SET JOIN_MEET_DAY = ?, JOIN_MEET_TIME_START = ?, JOIN_MEET_TIME_END = ?, JOIN_MEET_TIME_MARK = ?, JOIN_EDIT_TIME = ? WHERE JOIN_ID = ?')
