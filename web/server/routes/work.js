...
function getOwnedMeet(req, meetId) {
...
router.post('/login', (req, res) => {
  ...
  const user = db.prepare('SELECT * FROM users WHERE USER_MOBILE = ?').get(phone)
  ...
  db.prepare('UPDATE users SET USER_LOGIN_CNT = USER_LOGIN_CNT + 1, USER_LOGIN_TIME = ? WHERE USER_ID = ?').run(Date.now(), user.USER_ID)
...
// Work home stats
router.get('/home', authWork, (req, res) => {
  const today = todayLocal()
  const meets = listTeacherMeets(req)
  ...
  const todayJoinCount = db.prepare(`SELECT COUNT(*) as cnt FROM joins WHERE JOIN_MEET_ID IN (${placeholders}) AND JOIN_MEET_DAY = ? AND JOIN_STATUS = 1`).get(...meetIds, today).cnt
  const totalJoinCount = db.prepare(`SELECT COUNT(*) as cnt FROM joins WHERE JOIN_MEET_ID IN (${placeholders}) AND JOIN_STATUS = 1`).get(...meetIds).cnt
  const checkinCount = db.prepare(`SELECT COUNT(*) as cnt FROM joins WHERE JOIN_MEET_ID IN (${placeholders}) AND JOIN_IS_CHECKIN = 1`).get(...meetIds).cnt
  res.json({ data: { todayJoinCount, totalJoinCount, checkinCount } })
})

// List teacher's courses
router.get('/meets', authWork, (req, res) => {
  res.json({ data: listTeacherMeets(req) })
})

// Get meet detail for teacher
router.get('/meet/:id', authWork, (req, res) => {
  const meet = getOwnedMeet(req, req.params.id)
...
// Update meet
router.put('/meet/:id', authWork, (req, res) => {
  if (!getOwnedMeet(req, req.params.id)) return res.status(404).json({ msg: '未找到' })
  const { MEET_TITLE, MEET_STATUS } = req.body
  db.prepare('UPDATE meets SET MEET_TITLE = ?, MEET_STATUS = ?, MEET_EDIT_TIME = ? WHERE MEET_ID = ?')
    .run(MEET_TITLE, MEET_STATUS, Date.now(), req.params.id)
  res.json({ data: {} })
})

// Days
router.get('/meet/:id/days', authWork, (req, res) => {
  if (!getOwnedMeet(req, req.params.id)) return res.status(404).json({ msg: '未找到' })
...
router.post('/meet/:id/days', authWork, (req, res) => {
  if (!getOwnedMeet(req, req.params.id)) return res.status(404).json({ msg: '未找到' })
...
router.delete('/meet/:id/days/:dayId', authWork, (req, res) => {
  if (!getOwnedMeet(req, req.params.id)) return res.status(404).json({ msg: '未找到' })
  // Auto-cancel active bookings on this day and restore credits
  const dayRow = db.prepare('SELECT * FROM days WHERE DAY_ID = ? AND DAY_MEET_ID = ?').get(req.params.dayId, req.params.id)
  if (dayRow) {
    const activeJoins = db.prepare('SELECT * FROM joins WHERE JOIN_MEET_ID = ? AND JOIN_MEET_DAY = ? AND JOIN_STATUS = 1').all(req.params.id, dayRow.day)
    const now = Date.now()
  ...
  db.prepare('DELETE FROM days WHERE DAY_ID = ? AND DAY_MEET_ID = ?').run(req.params.dayId, req.params.id)
  res.json({ data: {} })
})

// Update a time slot's limit
router.put('/meet/:id/days/:dayId/slot/:mark', authWork, (req, res) => {
  if (!getOwnedMeet(req, req.params.id)) return res.status(404).json({ msg: '未找到' })
...
router.put('/meet/:id/days/:dayId/slot/:mark/time', authWork, (req, res) => {
  if (!getOwnedMeet(req, req.params.id)) return res.status(404).json({ msg: '未找到' })
  try {
...
router.get('/meet/:id/joins-by-slot', authWork, (req, res) => {
  if (!getOwnedMeet(req, req.params.id)) return res.status(404).json({ msg: '未找到' })
...
router.delete('/meet/:id/days/:dayId/slot/:mark', authWork, (req, res) => {
  if (!getOwnedMeet(req, req.params.id)) return res.status(404).json({ msg: '未找到' })
  const dayRow = db.prepare('SELECT * FROM days WHERE DAY_ID = ? AND DAY_MEET_ID = ?').get(req.params.dayId, req.params.id)
  if (!dayRow) return res.status(404).json({ msg: '日期不存在' })
...
router.get('/meet/:id/joins', authWork, (req, res) => {
  if (!getOwnedMeet(req, req.params.id)) return res.status(404).json({ msg: '未找到' })
  const rows = db.prepare(`
    SELECT joins.*, users.USER_NAME, users.USER_MOBILE 
    FROM joins 
...
router.post('/joins/:id/checkin', authWork, (req, res) => {
  const join = db.prepare('SELECT * FROM joins WHERE JOIN_ID = ?').get(req.params.id)
  if (!join) return res.status(404).json({ msg: '預約不存在' })
  if (!getOwnedMeet(req, join.JOIN_MEET_ID)) return res.status(403).json({ msg: '無權操作' })
  db.prepare('UPDATE joins SET JOIN_IS_CHECKIN = 1, JOIN_CHECKIN_TIME = ? WHERE JOIN_ID = ?').run(Date.now(), req.params.id)
  res.json({ data: {} })
})

router.post('/joins/:id/cancel', authWork, (req, res) => {
  ...
  if (!getOwnedMeet(req, join.JOIN_MEET_ID)) return res.status(403).json({ msg: '無權操作' })
  db.prepare('UPDATE joins SET JOIN_STATUS = 99, JOIN_EDIT_TIME = ? WHERE JOIN_ID = ?').run(Date.now(), join.JOIN_ID)
