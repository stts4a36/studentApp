...
function refundJoin(db, join, reason) {
  const now = Date.now()
  db.prepare('UPDATE joins SET JOIN_STATUS = 99, JOIN_REASON = ?, JOIN_EDIT_TIME = ? WHERE JOIN_ID = ?')
    .run(reason, now, join.JOIN_ID)

  const user = db.prepare('SELECT USER_LESSON_TOTAL_CNT, USER_LESSON_USED_CNT FROM users WHERE USER_ID = ?').get(join.JOIN_USER_ID)
  if (user) {
    const lastCnt = user.USER_LESSON_TOTAL_CNT
    const newCnt = lastCnt + 1
    const used = Math.max(0, (user.USER_LESSON_USED_CNT || 0) - 1)
    db.prepare('UPDATE users SET USER_LESSON_TOTAL_CNT = ?, USER_LESSON_USED_CNT = ? WHERE USER_ID = ?')
      .run(newCnt, used, join.JOIN_USER_ID)
    const logId = uuidv4()
    db.prepare(`INSERT INTO lesson_logs (LESSON_LOG_ID, LESSON_LOG_USER_ID, LESSON_LOG_MEET_ID, LESSON_LOG_DESC, LESSON_LOG_TYPE, LESSON_LOG_CHANGE_CNT, LESSON_LOG_LAST_CNT, LESSON_LOG_NOW_CNT, LESSON_LOG_ADD_TIME, LESSON_LOG_EDIT_TIME)
      VALUES (?, ?, ?, ?, 12, 1, ?, ?, ?, ?)`)
      .run(logId, join.JOIN_USER_ID, join.JOIN_MEET_ID, reason, lastCnt, newCnt, now, now)
  }
}

export function applySlotTimeChange(db, { meetId, dayId, mark, newDay, start, end, studentAction }) {
  ...
  const dayRow = db.prepare('SELECT * FROM days WHERE DAY_ID = ? AND DAY_MEET_ID = ?').get(dayId, meetId)
  if (!dayRow) {
  ...
  if (enrolled.length > 0 && studentAction === 'refund') {
    for (const join of enrolled) {
      refundJoin(db, join, '教師更改課堂時間，已退還課時')
    }
  ...
  let targetRow = targetDay === dayRow.day
    ? dayRow
    : db.prepare('SELECT * FROM days WHERE DAY_MEET_ID = ? AND day = ?').get(meetId, targetDay)
  ...
  if (targetDay === dayRow.day) {
    ...
    if (remaining.length === 0) {
      db.prepare('DELETE FROM days WHERE DAY_ID = ?').run(dayRow.DAY_ID)
    } else {
      db.prepare('UPDATE days SET times = ?, DAY_EDIT_TIME = ? WHERE DAY_ID = ?')
  ...
  if (enrolled.length > 0 && studentAction === 'move') {
    for (const join of enrolled) {
      db.prepare('UPDATE joins SET JOIN_MEET_DAY = ?, JOIN_MEET_TIME_START = ?, JOIN_MEET_TIME_END = ?, JOIN_EDIT_TIME = ? WHERE JOIN_ID = ?')
